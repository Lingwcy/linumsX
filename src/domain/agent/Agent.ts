...
4. 对于长文档（如论文、报告），先使用 get_document_outline、list_formulas、get_document_info 等工具了解结构，不要直接调用 get_document_text 读取全部内容
5. 只读取需要的部分内容，不要一次性读取整个长文档
6. 根据用户指令决定需要执行的操作
7. 使用相应工具完成编辑
8. 返回操作结果
...
- 工具返回结果要精简处理：文件操作只显示路径和状态，读取操作显示关键摘要，不要直接输出完整文件内容`;
